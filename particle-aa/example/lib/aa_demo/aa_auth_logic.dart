...
class AAAuthLogic {
  ...
  static void isSupportChainInfo() async {
    var result = await ParticleAA.isSupportChainInfo(ArbitrumChain.one());
    print(result);
  ...
  static void signAndSendTransactionWithNative() async {
    ...
    final signature = await ParticleAuth.signAndSendTransaction(transaction, feeMode: BiconomyFeeMode.native(result));
  ...
  static void signAndSendTransactionWithGasless() async {
    ...
    final signature = await ParticleAuth.signAndSendTransaction(transaction, feeMode: BiconomyFeeMode.gasless(result));
    print("signature $signature");
    showToast("signature $signature");
  }
  ...
  static void signAndSendTransactionWithToken() async {
    ...
    final signature = await ParticleAuth.signAndSendTransaction(transaction, feeMode: BiconomyFeeMode.token(feeQuote, tokenPaymasterAddress));
    print("signature $signature");
    showToast("signature $signature");
  }

  static void loginParticle() async {
    final result = await ParticleAuth.login(LoginType.email, "", [SupportAuthType.all]);
    print(result);
  }
  ...
  static void batchSendTransactions() async {
    ...
    final signature = await ParticleAuth.batchSendTransactions(transactions, feeMode: BiconomyFeeMode.native(result));
    print("signature $signature");
    showToast("signature $signature");
  }
}
